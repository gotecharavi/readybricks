...
function OffersCtrl($scope, $http){	
	...
	$scope.saveItem=function(){	
		...
		if($scope.item==null || $scope.item=="" ){
            ...
            $scope.errors.imageMsg = 'Please select image.';
            return false;
        }
...
//    console.log($scope.totalpaging);
	...
	function loadData(action,data){
		return $http({
			  method: 'POST',
			  url: BASE_URL+'Offers_ctrl/'+action,
			  data: data,
 ...
 OffersCtrl.prototype.init=function($scope){
	$scope.search=null;
	$scope.item=null;
	$scope.list = null;
	$scope.fgShowHide=true;
...
OffersCtrl.prototype.configureGrid=function($scope){
	$scope.totalItems = 0;
    $scope.pagingOptions = {
...
OffersCtrl.prototype.searchPopup=function($scope){
	$scope.showForm=function(){$scope.fgShowHide=false; $scope.item=null; $scope.datatype= 'Add';};
